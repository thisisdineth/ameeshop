document.addEventListener('DOMContentLoaded', () => {
    ...
    async function loadCustomerDetails(customerId) {
        ...
        if (!pageTitleEl || !customerCardTitleEl || !customerDetailContainerEl) {
    ...
    function displayCustomerInfo(customerId, data) {
        ...
        } else if (phoneNumbersContainerEl) {
            addPhoneNumberField(); 
        }
    }
    ...
    if (customerDetailsForm) {
        customerDetailsForm.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                if(pageTitleEl && customerNameInput) pageTitleEl.innerHTML = `<i class="fas fa-user-tie fa-fw"></i> Customer: ${customerNameInput.value.trim()}`;
            } catch (error) {
    ...
    function renderSaleRowForCustomer(saleData) {
        ...
        // --- CHANGE: Pointing to the new receipt function ---
    ...
    // --- NEW: Replaced the old A4 invoice function with the 80mm thermal receipt function ---
    async function generateAndPrintReceipt(saleData) {
        try {
            ...
            (saleData.items || []).forEach(item => {
                ...
                doc.text('0.00', 45, currentY); // Per-item discount is not in the data model
